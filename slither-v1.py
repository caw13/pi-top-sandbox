...
class Snake:
    def __init__(self, canvas, color,key_press_left, key_press_right, key_press_up, key_press_down):
        ...
        self.turn_increment = 2.5
        self.length_increment = 400
        self.length = self.length_increment
        self.canvas = canvas
        self.x_speed = self.speed_increment
        self.y_speed = 0
        self.color = color
        first_piece = canvas.create_oval(0, 0, 10, 10, fill=color)
        ...
        self.head = canvas.create_oval(pos[0], pos[1], pos[2], pos[3], fill=color)
        self.hit_wall = False
    ...
    def turn_left(self, event):
        if self.x_speed >= 0 and self.y_speed >= 0:
            self.x_speed = self.speed_govenor(self.x_speed + self.turn_increment)
            self.y_speed = self.speed_govenor(self.y_speed - self.turn_increment)
    ...
    def draw(self):
        for i in range(0,self.max_speed):
            old_pos = self.canvas.coords(self.head)
            new_piece = canvas.create_oval(old_pos[0], old_pos[1], old_pos[0]+10, old_pos[1]+10, fill=self.color, outline=self.color)
            ...
            self.canvas.move(self.head, self.x_speed/self.max_speed, self.y_speed/self.max_speed)
            ...
            if self.hit_food(pos):
                self.length = self.length + self.length_increment
    ...
    def hit_food(self, current_pos):
        ...
        for food in self.food_array:
            if head_pos[0] <= food.pos_x and head_pos[2]>= food.pos_x and head_pos[1] <= food.pos_y and head_pos[3] >= food.pos_y:
                return True
        return False
...
class Food:
    def __init__(self, canvas, snake, width, height):
        ...
        color = random_color()
        self.shape = canvas.create_oval(self.pos_x-2.5, self.pos_y-2.5, self.pos_x + 2.5, self.pos_y + 2.5, fill=color,outline=color )
...
def begin_new_game(event):
    ...
    if not snake == None:
        snake.remove()
    snake = Snake(canvas, random_color(),"<KeyPress-Left>","<KeyPress-Right>","<KeyPress-Up>","<KeyPress-Down>")
